class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        ...
        Sliding window using two pointer, with left at 0 and right at 1
        Update maxi with profit only if right is greater than left.
        if left is greater set left equal to right and increment right.
        Keep incrementing right irrespectuve of anything
        """
        left=0
        right=1
        maxi=0
